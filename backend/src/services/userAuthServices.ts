...
export class UserAuthService implements IUserAuthService {
  constructor(
    private _authRepository: IUserAuthRepository,
  ...
  async loginUser(email: string, password: string): Promise<LoginResult> {
    const user: IUser | null = await this._authRepository.findUserByEmail(
      email
    );
  ...
  async loginUserWithGoogle(credential: string): Promise<RegisterResult> {
    try {
      ...
      let user = await this._authRepository.findUserByEmail(email!);
