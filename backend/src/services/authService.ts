...
export class AuthService implements IAuthService {
  ...
  async passwordForgot({ email, userType }: Resend): Promise<RegisterResult> {
    try {
      ...
      const repo: ICommonAuthRepository =
        CommonAuthRepository.getRepository(userType);
      ...
      await this._emailService.sendMail(mailOptions);
  ...
  async resetPassword({
    ...
    password,
    userType,
  }: LoginBody): Promise<RegisterResult> {
    try {
