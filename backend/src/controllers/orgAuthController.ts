...
export class OrgAuthController implements IOrgAuthController {
  constructor(
    private _authService: IOrgAuthService,
    private setTokenService: ISetTokenService
  ) {}
  async organiserLogin(
    req: Request<unknown, unknown, LoginBody>,
    res: Response
  ): Promise<void> {
    try {
      const { email, password } = req.body;
      const result: LoginResult = await this._authService.loginOrganiser(
        email,
        password
      );
  ...
  async organiserRegister(
    req: Request<unknown, unknown, RegisterBody>,
    res: Response
  ): Promise<void> {
    try {
      const { name, email, password } = req.body;
      const result: RegisterResult = await this._authService.registerOrganiser(
        name,
        email,
        password
      );
  ...
  async organiserVerify(
    ...
    try {
      ...
      const result: RegisterResult = await this._authService.organiserVerify({
        name,
        email,
        password,
        otp,
  ...
  async organiserResend(
    ...
    try {
      ...
      const result: RegisterResult = await this._authService.resendOrganiser({
        email,
      });
  ...
  async googleOrganiserLogin(
    req: Request<unknown, unknown, CredentialResponse>,
    res: Response
  ): Promise<void> {
    try {
      ...
      const result: RegisterResult =
        await this._authService.loginOrganiserWithGoogle(credential);
      if (!result.success) {
