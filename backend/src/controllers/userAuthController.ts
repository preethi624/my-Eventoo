...
export class UserAuthController implements IUserAuthController {
  ...
  async userLogin(
    ...
    try {
      ...
      const result: LoginResult = await this._authService.loginUser(
        email,
        password
      );
  ...
  async userRegister(
    ...
    try {
      ...
      const result: RegisterResult = await this._authService.registerUser(
        name,
        email,
        password
      );
  ...
  async userVerify(
    ...
    try {
      ...
      const result: RegisterResult = await this._authService.userVerify({
        name,
        email,
        password,
        otp,
  ...
  async userResend(
    ...
    try {
      ...
      const result: RegisterResult = await this._authService.resendUser({
        email,
      });
  ...
  async googleUserLogin(
    ...
    try {
      ...
      const result: RegisterResult =
        await this._authService.loginUserWithGoogle(credential);
      if (!result.success) {
  ...
  async handleRefreshToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      const newToken = await this._authService.refreshTokens(refreshToken);
