...
const registerOrganiser = async (
  ...
  try {
    const response = await axios.post(
      `${API_BASE_URL}/organiser/register`,
      credentials
    );
...
const resendOtp = async ({ email }: ResendPayload): Promise<ApiResponse> => {
  try {
    const response = await axios.post(`${API_BASE_URL}/user/resendOtp`, {
      email,
    });
...
const verifyOtpOrganiser = async (
  credentials: VerifyOtpOrgPayload
): Promise<ApiResponse> => {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/organiser/otp`,
      credentials
    );
    return response.data;
  } catch (error) {
