...
const [selectedYear,setSelectedYear]=useState("")
 ...
 const[organiserData,setOrganiserData]=useState({
  ...
  // Current organizer context (replace with actual user data from your auth system)
   ...
   const fetchEvents=async(timeFrame:string,startDate?:string,endDate?:string,selectedCategory?:string)=>{
  ...
  if(selectedYear){
    ...
    const response=await organiserRepository.getDashboardEvents(organiser.id,params);
...
const exportToCSV = (orders: any[], filename: string,adminPercentage:number) => {
...
const exportToPDF = (orders: any[], filename: string, adminPercentage: number) => {
  const doc = new jsPDF();
  const today = new Date();
  ...
  const tableRows = orders.map(order => {
    ...
    // Calculate total only for past events
    if (order.eventDate && new Date(order.eventDate) < today) {
      totalEarnings += organiserEarning;
    }
    ...
    return [
      ...
      `₹${commission.toFixed(2)} (${adminPercentage}%)`,
...
<div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/30 mb-12 hover:shadow-3xl transition-all duration-500">
  ...
  <button onClick={()=>fetchEvents(selectedTimeframe,fromDate,toDate,selectedCategory)}
 ...
 {availableCategories.map((cat: { _id: string; name: string }) => (
  <option key={cat._id} value={cat.name}>
    {cat.name}
            ...
            <button className="bg-blue-500 text-white px-6 py-1 rounded-xl hover:bg-blue-700 transition-colors flex items-center" onClick={() => exportToCSV(orders, "dashboard_report",adminPercentage)}>
  ...
  onClick={() => exportToPDF(orders, "dashboard_report", adminPercentage)}
>
  <Download size={16} className="mr-2" />
  Export PDF Report
</button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={revenueData.map((d) => ({ ...(d as { month: string; revenue: number }), revenue: d.revenue / 100 }))}>
                <defs>
