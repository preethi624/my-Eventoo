          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className={`px-2 py-1 rounded-full text-sm ${
                event.status === 'draft' ? 'bg-green-100 text-yellow-800' :
                event.status === 'published' ? 'bg-blue-100 text-blue-800' :
                event.status === 'cancelled' ? 'bg-gray-100 text-gray-800' :
                event.status === 'completed' ? 'bg-gray-100 text-green-800' :
                'bg-red-100 text-red-800'
              }`}>
                {event.status}
              </span>
            </div>
            <p className="text-gray-600 mt-2">{expandedEvents[event._id]?event.description:truncate(event.description,100)}</p>
            <button onClick={()=>toggleTrunk(event._id)}>{expandedEvents[event._id]?'Show less':'Read more'}</button>
            ...
            <div className="mt-4 flex space-x-2">
              <button
                onClick={() => {
                  ...
                  setFormData({title:event.title,description:event.description,date:eventDate.toISOString().split("T")[0],venue:event.venue,capacity:event.capacity,status:event.status,time:event.time,ticketPrice:event.ticketPrice})
                  setIsModalOpen(true);
                }}
              ...
              <button
                    ...
                    className={`px-3 py-1 rounded text-sm font-medium ${
                      event.isBlocked ? 'bg-green-500 text-white' : 'bg-yellow-500 text-black'
                    }`}
                  >
...
/>
  ...
  onChange={(e) => setFormData({ ...formData, status: e.target.value as 'draft' | 'published' | 'completed'|'cancelled' })}
  className="w-full px-4 py-2 border rounded"
>
